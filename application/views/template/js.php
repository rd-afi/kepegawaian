</div><!-- /.content-wrapper -->

<footer class="main-footer">
    <div class="pull-right hidden-xs">

    </div>
    <strong>Copyright &copy; 2017 <a href=""></a>.</strong> All rights reserved.
</footer>
</div><!-- ./wrapper -->

<!-- jQuery 2.1.3 -->
<script src="<?php echo base_url('assets/template/plugins/jQuery/jquery-2.1.4.min.js') ?>"></script>
<!-- Sweetalert -->
<!-- <script src="<?php echo base_url('assets/template/plugins/bootstrap-sweetalert/dist/sweetalert.min.js') ?>" type="text/javascript"></script> -->
<!-- <script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert.min.js"></script> -->
<!-- Alertify -->
<script src="<?php echo base_url('assets/template/plugins/alertify.js/dist/js/alertify.js') ?>" type="text/javascript"></script>
<!-- Bootstrap 3.3.2 JS -->
<script src="<?php echo base_url('assets/template/bootstrap/js/bootstrap.min.js') ?>" type="text/javascript"></script>
<!-- SlimScroll -->
<script src="<?php echo base_url('assets/template/plugins/slimScroll/jquery.slimScroll.min.js') ?>" type="text/javascript"></script>
<!-- FastClick -->
<script src='<?php echo base_url('assets/template/plugins/fastclick/fastclick.min.js') ?>'></script>
<!-- iCheck 1.0.1 -->
<script src='<?php echo base_url('assets/template/plugins/iCheck/icheck.min.js') ?>'></script>
<!-- bootstrap datepicker -->
<script src='<?php echo base_url('assets/template/plugins/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js') ?>'></script>
<!-- AdminLTE App -->
<script src="<?php echo base_url('assets/template/dist/js/app.min.js') ?>" type="text/javascript"></script>
<!-- DataTables -->
<!-- <script src="<?php echo base_url('assets/template/plugins/datatables/datatables.net/js/jquery.dataTables.min.js') ?>" type="text/javascript"></script> -->
<!-- <script src="<?php echo base_url('assets/template/plugins/datatables/datatables.net-bs/js/dataTables.bootstrap.min.js') ?>" type="text/javascript"></script> -->
<!-- DataTables -->
<!-- <script src="<?php echo base_url('assets/template/plugins/datatables/DataTables/js/jquery.dataTables.min.js') ?>" type="text/javascript"></script> -->
<!-- <script src="<?php echo base_url('assets/template/plugins/datatables/DataTables/js/dataTables.bootstrap.min.js') ?>" type="text/javascript"></script> -->
<script type="text/javascript" src="<?php echo base_url('assets/template/plugins/datatables/datatables.min.js') ?>"></script>
